...
public class GetData {
	...
	public static void main(String[] args) {
		...
		l.stream().sorted(Comparator.comparing(Employee::getName)).collect(Collectors.toList())
		...
		/*
		 * l.stream().collect(Collectors.groupingBy(Employee::getAddress)).entrySet().
		 * stream()
		 * .sorted(Collections.reverseOrder(Map.Entry.comparingByKey())).forEach(System.
		 * out::println);
		 */
		
		l.stream().collect(Collectors.groupingBy(Employee::getAddress)).entrySet()
		.stream().sorted(Collections.reverseOrder(Map.Entry.comparingByKey())).forEach(System.out::println);
	}
